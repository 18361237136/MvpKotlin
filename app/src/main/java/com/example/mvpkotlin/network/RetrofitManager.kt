...
import com.example.mvpkotlin.network.api.ApiService
import com.example.mvpkotlin.network.api.UrlConstant
import com.example.mvpkotlin.utils.AppUtils
import com.example.mvpkotlin.utils.NetWorkUtil
import com.example.mvpkotlin.utils.Preference
import okhttp3.*
 ...
 * Date: 2019/3/8 上午11:34
 * Describe:单例模式的网络请求封装
 */
object RetrofitManager {
    ...
    private fun addCacheInterceptor():Interceptor{
        return Interceptor {chain ->
            ...
            if(NetWorkUtil.isNetworkAvailable(MyApplication.context)){
                val maxAge=0
                //有网络时设置缓存超时时间0小时，意思就是不读取缓存数据，支队get有用，post没有缓存
                response.newBuilder()
                        .header("Cache-Control", "public, max-age=" + maxAge)
                        .removeHeader("Retrofit")
                        .build()
            }else{
                //无网络是设置超时为4周,只对get有用,post没有缓冲
